...
with tempfile.TemporaryDirectory() as temp_dir:
    ...
    for obj in objetos_bronze:
        ...
        if col_comments in df.columns:
            ...
            df.rename(columns={col_comments: "comments"}, inplace=True)
    ...
    df_silver["title_length"] = df_silver["title"].astype(str).apply(len)
    df_silver["num_tags"] = df_silver["tags"].astype(str).apply(lambda x: len(x.split(",")))
    df_silver["description_length"] = df_silver["description"].astype(str).apply(len)

    safe_views = df_silver["views"].replace(0, 1)

    df_silver["likes_ratio"] = df_silver["likes"] / safe_views
    df_silver["comments_ratio"] = df_silver["comments"] / safe_views

    df_silver["viral"] = (df_silver["views"] > 1_000_000).astype(int)
    ...
    corr_path = os.path.join(temp_dir, "correlation.csv")
    corr_matrix.to_csv(corr_path)

    client.fput_object("silver", "correlation.csv", corr_path)
